...
（1）每个GPU吃一份数据，进行前向后向传播得到梯度，对GPU0-2 的梯度需要进行reduce scatter 进行聚合平均，例如：GPU0和1把后层的FP16的梯度发给GPU2

（2）获得梯度均值后转为FP32  然后更新FP32的参数（优化器中）

（3）接下来更新FP16的网络参数

（4） 前中后的FP16参数已经更新 需要一步all-gather 广播到其他网络
...
升级后梯度也分开 GPU0-2都反向计算梯度  但是只保存只需要的梯度 从后到前更新梯度 按照桶的形式发送给负责更新的GPU其他的发送后就释放显存

这样每个GPU都有了梯度 然后更新优化器状态、优化器更新FP16参数、再广播参数即可（与上面一致）
...
参数也划分了 前向传播遇到自己没有的参数就靠其他的GPU广播 反向也是
