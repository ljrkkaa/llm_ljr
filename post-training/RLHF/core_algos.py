...
来自https://github.com/volcengine/verl/blob/main/verl/trainer/ppo/core_algos.py 实现了PPO GRPO REINFORCE++-baseline REINFORCE++等的优势函数
    ...
    with torch.no_grad():
        ...
        for idx in id2score:        # 求均值
            if len(id2score[idx]) == 1:
                ...
                raise ValueError(f"no score in prompt index: {idx}")
        for i in range(bsz):    # 按样本组（prompt index）先做局部中心化
            scores[i] = scores[i] - id2mean[index[i]] 

        # .tile() 是用于 沿指定维度重复张量 的方法 0维1次 1维response_length次
        scores = scores.unsqueeze(-1).tile([1, response_length]) * response_mask  
        # 按 token 级别对整个 batch 做标准化
